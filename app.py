...
if g_df is not None and len(g_df) > 2:
    ...
    # --- تعديل الخطوط (آسيا منقطة مثل لندن) ---
    fig.add_hline(y=mn_open, line_color="red", line_width=3, annotation_text="MIDNIGHT")
    
    # خطوط آسيا (أزرق سماوي منقط مثل لندن)
    if ah > 0:
        fig.add_hline(y=ah, line_color="#00FFFF", line_width=2, line_dash="dot", annotation_text="ASIA HIGH")
        fig.add_hline(y=al, line_color="#00FFFF", line_width=2, line_dash="dot", annotation_text="ASIA LOW")
    ...
    # --- تعديل أزرار التحكم للتابلت (كبيرة، سوداء، متباعدة) ---
    fig.update_layout(
        template="plotly_dark", height=700, margin=dict(l=0,r=0,t=0,b=0),
        xaxis_rangeslider_visible=False,
        modebar=dict(
            bgcolor='black',      # خلفية سوداء للأزرار
            color='white',        # لون الأيقونات
            activecolor='#00FFFF', # لون الزر عند الضغط
            orientation='h',      # ترتيب أفقي
            uirevision=True       # الحفاظ على الزووم عند التحديث
        )
    )
    
    # إعدادات العرض الخاصة باللمس
    st.plotly_chart(fig, use_container_width=True, config={
        'scrollZoom': True, 
        'displayModeBar': True,
        'displaylogo': False,
        'doubleClick': 'reset+autosize', # إعادة الضبط بلمستين
        'modeBarButtonsToRemove': ['lasso2d', 'select2d'], # إزالة الأزرار غير الضرورية لتقليل الزحام
        'responsive': True
    })
