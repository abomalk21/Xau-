...
def get_gold_only():
    try:
        # سحب بيانات الذهب فقط لضمان السرعة
...
if g_df is not None and not g_df.empty:
    ...
    # 1. تنبيه الأخبار (إصلاح العرض البصري)
    st.warning("⚠️ **Friday Alert:** High Volatility. Watch for NFP/Jobs Report (08:30 AM NY).")

    # 2. المربعات العلوية (تم إصلاح خطأ TypeError هنا)
    ...
    # 3. الشارت (توحيد آسيا ولندن أصفر منقط)
    ...
    # 4. السطر السفلي (المواقيت فقط لضمان السرعة)
    st.markdown("---")
